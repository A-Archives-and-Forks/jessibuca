class MP4 {
  ...
  // emit ftyp & moov
  static generateInitSegment(meta: any, trakList: any, mdatBytes: number) {
  ...
  // Movie metadata box
  // Moov box 主要包含 mvhd、trak、mvex三种子box。
  static moov(meta: any, trakList: any) {
  ...
  static stbl(trak: any) {
    ...
    return MP4.box(MP4.types.stbl,  // type: stbl
      MP4.stsd(trak),  // Sample Description Table
  ...
  static stco(sampleToChunk: any, mdatDataList: any) {
  ...
  // Movie fragment box
  static moof(track: any, baseMediaDecodeTime: number) {
  ...
  // Track fragment box
  static traf(track: any, baseMediaDecodeTime: number) {
  ...
  static trun(track: any, offset: number) {
    let samples = track.samples || [];
    let sampleCount = samples.length;
